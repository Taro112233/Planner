...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      role="group"
      className={cn(
        "group/input-group border-input dark:bg-input/30 relative flex w-full items-center rounded-xl border shadow-xs transition-all duration-200 outline-none",
...
const inputGroupButtonVariants = cva(
  "text-sm shadow-none flex gap-2 items-center transition-all duration-200 active:scale-[0.97]",
  {
    variants: {
      size: {
...
function InputGroupInput({
  ...
  return (
    <input
      ...
      className={cn(
        "flex h-9 w-full flex-1 rounded-none border-0 bg-transparent px-3 py-1 text-sm shadow-none transition-colors duration-200 file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-transparent",
        className
      )}
      {...props}
    />
